...
const CLOUD_CONFIG_KEYS = ["todoList", "wallpaperConfig", "tabSettings"];
...
async function backupData() {
    try {
        const backup = { localStorage: {}, indexedDB: {}, cloud: {} };
        ...
        // Backup IndexedDB (ImageDB) — legacy,保留兼容（壁纸已改用 URL）
        backup.indexedDB = await backupIndexedDB();

        // Backup Cloud Config (if available)
        if (typeof loadCloudConfig === "function") {
            for (const key of CLOUD_CONFIG_KEYS) {
                try {
                    const val = await loadCloudConfig(key);
                    backup.cloud[key] = val;
                } catch (err) {
                    console.warn("backup cloud config failed", key, err);
                }
            }
        }
...
async function restoreData(backup) {
    ...
    // Restore IndexedDB from backup (legacy)
    if (backup.indexedDB) {
        await restoreIndexedDB(backup.indexedDB);
    }

    // Restore cloud config (if available)
    if (backup.cloud && typeof saveCloudConfig === "function") {
        for (const key of Object.keys(backup.cloud)) {
            try {
                await saveCloudConfig(key, backup.cloud[key]);
            } catch (err) {
                console.warn("restore cloud config failed", key, err);
            }
        }
    }
}
